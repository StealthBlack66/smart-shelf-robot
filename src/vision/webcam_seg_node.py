#!/usr/bin/env python3
"""Webcam Segmentation ROS2 Node.

모델 구성:
  YOLO11s-seg (pose_robust_seg)  — 메인 검출·분할·분류 (4-class: bottle/can/snack_bag)
  GroundingDINO SwinT            — open-vocab 보조 검출 (3s 주기 백그라운드)
  SAM2 Hiera Large               — 정밀 마스크 (ENABLE_SAM2=1, 백그라운드 워커)
  rembg ISNet (ONNX, CPU)        — SAM2 없을 때 fallback

데이터 흐름:
  RealSense → [BG] GD(3s) + SAM2(1.5s) → [Main] YOLO seg → vis + ROS publish
  'g' = GraspGen export / 'p' = pick_pose 발행 → curobo_planner_node

실행:
  ENABLE_SAM2=1 python3 webcam_seg_node.py
  (curobo_planner_node + gripper_service_node + dsr_bringup2 필요)
"""
import os
import sys
import subprocess
...
GRASPGEN_LABELS = ('snack_bag', 'can', 'bottle')   # 확정 enum (단위 meter, base frame)
...
MARKER_LABELS = {'pick': 'PICK', 'place': 'PLACE'}
...
class WebcamSegNode(Node):
    ...
    def _load_vision_models(self, weights):
        ...
        if self.hqsam_predictor is None and os.environ.get('ENABLE_SAM2', '1') != '0':
            for ckpt, cfg, label in [
                (os.path.join(os.path.dirname(os.path.abspath(__file__)), 'models', 'sam2_hiera_large.pt'), 'configs/sam2/sam2_hiera_l.yaml', 'Large'),
                (os.path.join(os.path.dirname(os.path.abspath(__file__)), 'models', 'sam2_hiera_small.pt'), 'configs/sam2/sam2_hiera_s.yaml', 'Small'),
                (os.path.join(os.path.dirname(os.path.abspath(__file__)), 'models', 'sam2_hiera_tiny.pt'),  'configs/sam2/sam2_hiera_t.yaml', 'Tiny'),
            ]:
    ...
    def _init_state(self):
        ...
        self._det_track        = {}
        self.locked_tk         = None
        self.locked_class      = None   # 번호 선택 시점 클래스명 (고정)
        self.pending_grasp_pose = None
        self.approach_advance  = 0.12
        self.gripper_offset    = 0.11
        self.pregrasp_standoff = 0.06
        self.pending_grasp_standoff = None   # snack_bag 수직하강용 custom standoff
        _gfz = os.environ.get('WSN_GRASP_FIXED_Z', '57.5')
        self.grasp_fixed_z = (None if _gfz in ('auto', 'off', '') else float(_gfz) / 1000.0)
        self.gg = None
        if _GRASPGEN_AVAIL:
            try:
                gg_host = os.environ.get('GRASPGEN_HOST', 'localhost')
                gg_port = int(os.environ.get('GRASPGEN_PORT', '5556'))
                self.gg = GraspGenClient(gg_host, gg_port,
                                         timeout_ms=60000, wait_for_server=False)
                self.get_logger().info(f'GraspGen 클라이언트 준비 ({gg_host}:{gg_port})')
            except Exception as e:
    ...
    def _setup_window(self):
        self.win = 'webcam_seg_node (`+w=PICK `+s=PLACE c=clear f=full m=refine ESC=quit)'
    ...
    def _set_snackbag_grasp(self, det):
        """과자봉지: GraspGen 없이 검출 중심에서 수직 하강 파지 계산.
        approach = -Z (위에서 아래로), 핑거축 = 봉지 장축(rz). 's'에서 10cm 위 프리그래스프."""
        C = np.asarray(det.get('center_base'), dtype=float)
        approach = np.array([0.0, 0.0, -1.0])
        # 봉지 장축 방향으로 핑거 정렬 (rz = base XY 평면 내 회전각)
        bbox = det.get('bbox')
        cloud = det.get('cloud_m')
        rz_deg = 0.0
        if cloud is not None and bbox is not None:
            rxyz = self.cloud_to_rxryrz(
                cloud,
                key=self._isnet_grid_key(*bbox),
                cname='snack_bag')
            if rxyz is not None:
                rz_deg = float(rxyz[2])
        rz_rad = np.deg2rad(rz_deg)
        x = np.array([np.cos(rz_rad), np.sin(rz_rad), 0.0])
        y = np.cross(approach, x); y /= np.linalg.norm(y)
        R_grasp = np.column_stack([x, y, approach])
        quat = _rotm_to_quat(R_grasp)
        self.pending_grasp_pose = (C, np.asarray(quat), approach)
        self.pending_grasp_standoff = 0.10
        self.publish_grasp_marker(C, quat, action=Marker.ADD)
        self.get_logger().info(
            f"[g] snack_bag 수직하강 파지: "
            f"중심=({C[0]*1000:.0f},{C[1]*1000:.0f},{C[2]*1000:.0f})mm "
            f"rz={rz_deg:.1f}° 프리그래스프 +{self.pending_grasp_standoff*100:.0f}cm 위")
    ...
    def send_graspgen(self):
        ...
        try:
            grasps, confs = self.gg.infer(pc, remove_outliers=False)
        except Exception as e:
    ...
    def move_to_grasp(self):
        ...
        time.sleep(0.3)
        # 프리그래스프 = 파지중심에서 (그리퍼길이+standoff) 만큼 approach 반대로 뒤.
        _standoff = (self.pending_grasp_standoff if self.pending_grasp_standoff is not None
                     else self.pregrasp_standoff)
        pre = (np.asarray(C, dtype=float)
               - (self.gripper_offset + _standoff) * np.asarray(approach))
        # s 이동 시 base X축 추가 오프셋. 수직하강(snack_bag) 시 미적용.
        _sxo = float(os.environ.get('WSN_S_X_OFFSET', '0.03'))
        if self.pending_grasp_standoff is None:
            pre[0] += _sxo
    ...
    def advance_and_grip(self):
        ...
        _grasp_class = self.locked_class or (self._selected() or {}).get('name')
        if _grasp_class:
            self.pub_grasp_class.publish(String(data=str(_grasp_class)))
            self.get_logger().info(f"[p] 파지 클래스 발행: {_grasp_class} (locked_class={self.locked_class})")
        time.sleep(0.3)
        # 집기: 그리퍼밑동 = 캔중심 - 그리퍼길이*approach (손가락이 캔중심에 닿음).
        adv = np.asarray(C, dtype=float) - self.gripper_offset * np.asarray(approach)
        # base X축 추가 오프셋. 수직하강(snack_bag) 시 미적용.
        _sxo = float(os.environ.get('WSN_S_X_OFFSET', '0.03'))
        if self.pending_grasp_standoff is None:
            adv[0] += _sxo
    ...
    def clear_grasp_preview(self):
        if self.pending_grasp_pose is None:
            return False
        self.pending_grasp_pose = None
        self.pending_grasp_standoff = None
    ...
    def _render_sam_object(self, vis, frame, base_mask_full, bbox, cname,
        ...
        # GD-only 검출도 번호박스 대상에 추가 (YOLO가 놓친 경우 GD로 보완)
        _gd_center = np.asarray(base_xyz, dtype=float) / 1000.0
        _already = any(
            ((_d['center_base'][0] - _gd_center[0]) ** 2
             + (_d['center_base'][1] - _gd_center[1]) ** 2) ** 0.5 < 0.06
            for _d in self.detections)
        if not _already:
            self.detections.append({
                'name': cname,
                'gg_label': gg_label,
                'center_base': _gd_center,
        ...
        try:
            while rclpy.ok():
                ...
                if res is not None and res.masks is not None:
                    try:
                        ...
                        for mi in range(masks_data.shape[0]):
                            ...
                            mask01 = (refined > 0).astype(np.uint8)
                            _bbox_tk = self._track_key(
                                int(bbox[0]), int(bbox[1]), int(bbox[2]), int(bbox[3]))
                            out = self.mask_to_base_xyz(mask01, depth_arr)
                            if out is None:
                                _cached = getattr(self, '_bbox_det_cache', {}).get(_bbox_tk)
                                if _cached is not None:
                                    self.detections.append(_cached)
                                continue
                            cu, cv_y, base_xyz = out
                            ...
                            # flip 깜빡임 완화. eye-in-hand 이동에도 안정하도록
                            # base XY 6cm 격자 기준 (픽셀 기준은 팔 이동 시 리셋됨).
                            _cid = int(cls_ids[mi])
                            _bvk = (round(float(base_xyz[0]) / 60.0),
                                    round(float(base_xyz[1]) / 60.0))
                            if not hasattr(self, '_cls_vote'):
                                self._cls_vote = {}
                            _vv = self._cls_vote.setdefault(_bvk, [])
                            _vv.append(_cid)
                            if len(_vv) > 30:
                                _vv.pop(0)
                            ...
                            if not hasattr(self, '_bbox_det_cache'):
                                self._bbox_det_cache = {}
                            self._bbox_det_cache[_bbox_tk] = _det_entry
                            self.detections.append(_det_entry)
                ...
                try:
                    ...
                    rendered_keys = set()
                    _minc = float(self._otun('gd_only_min_conf', 0.30))
                    ...
                    for g in sorted(gd_snap,
                        ...
                        cn = ('snack_bag' if 'snack' in ph
                              ...
                              else 'can' if 'can' in ph else None)
                        if cn is None:
                            continue
                ...
                if self.pending_grasp_pose is not None:
                    ...
                    cv2.putText(vis,
                                f"GRASP PREVIEW ({_pb[0]*1000:.0f},{_pb[1]*1000:.0f},{_pb[2]*1000:.0f})mm  s=move p=advance+grasp r=cancel",
                ...
                if ord('1') <= k <= ord('9'):
                    ...
                    if _match is not None:
                        ...
                        self.locked_idx = None
                        self.locked_class = _match.get('name')
                        self.get_logger().info(
                ...
                if k == ord('c') and '`' not in self.keys_held:
                    pass  # reserved for future clear action
