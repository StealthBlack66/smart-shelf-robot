...
class ArmControllerNode(Node):
    ...
    def execute_spline(self, traj_rad, vel_scale: float = 1.0):
        ...
        for row in traj_deg:
            ...
            pt.data = row.tolist()
            req.pos.append(pt)
        vel_deg = 30.0 * self.VEL_SCALE * vel_scale
        req.vel = [vel_deg] * 6
        req.acc = [60.0] * 6
        ...
        t0 = time.time()
        while not future.done() and (time.time() - t0) < spline_timeout:
            time.sleep(0.05)
        ok = future.done() and future.result() and future.result().success
        self.get_logger().info("Spline " + ("완료" if ok else f"실패 (elapsed {time.time()-t0:.1f}s)"))
